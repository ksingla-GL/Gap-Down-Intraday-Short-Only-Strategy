...
for file in data:
    ...
    mult=df["Threshold"]*1.0005
    plus=df["Threshold"]+0.05
    ...
    df["new_Threshold"]=np.where(df["Threshold"]*1.0005>df["Threshold"]+0.05,df["Threshold"]*1.0005,\
    df["Threshold"]+0.05)
    df["new_SL"]=np.where(df["SL"]*0.9995<df["SL"]-0.05,df["SL"]*0.9995,df["SL"]-0.05)
    
    #Now we generate signals and a column for our current positions   
    df["Signal"].ix[(df["Low"]<=df["new_Threshold"])&(np.isfinite(df["new_Threshold"]))& \
    ...
    for date in sell_days:
        ...
        time_of_exit=new_day["Time"].ix[new_day["High"]>=new_day["new_SL"]]
        same_candle_exit=new_day["Time"].ix[(new_day["High"]>=new_day["new_SL"]) & \
