...
for file in data:
    ...
    df["Signal"].ix[(df["Low"]<=df["Threshold"]*1.0006)&(np.isfinite(df["Threshold"]))& \
    ...
    for date in sell_days:
        ...
        time_of_exit=new_day["Time"].ix[new_day["High"]>=new_day["SL"]*0.9994]
        same_candle_exit=new_day["Time"].ix[(new_day["High"]>=new_day["SL"]*0.9994) & \
