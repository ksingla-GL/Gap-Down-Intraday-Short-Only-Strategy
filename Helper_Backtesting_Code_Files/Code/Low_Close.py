...
os.chdir("Execution_low_close")
for file in data:
    #os.chdir("Output")
    x+=1
    df=data[file]
    df["Time"]=pd.to_datetime(df["Time"])
    df=df.ix[(df["Time"]<"2017-07-03 15:30:00") & (df["Time"]>"2017-07-03 09:00:00")]
    ...
    for i in range(len(dates)-1):
        day=df.ix[df["Date"]==dates[i]]
        
        #Now calculating the variables we need to compare to our signals
        open=day.iloc[0]["Open"]
        close=day.iloc[len(day)-1]["Close"]
        candle_open=day.iloc[len(day)-1]["Open"]
        
        #Adding these variables now as new columns to our df
        df.loc[(df["Date"]==day.iloc[len(day)-1]["Date"]) & (df["Time"]==day.iloc[len(day)-1]["Time"])\
        ,["Day_close"]]=close
        df.loc[(df["Date"]==day.iloc[0]["Date"]) & (df["Time"]==day.iloc[0]["Time"])\
        ,["Day_open"]]=open
    ...
    df["Time"]=pd.to_datetime(df["Time"])
    df["Threshold"]=np.where((df["Threshold"]>0)&(df["Close"]!=df["Low"]),df["Threshold"]*1.0006,\
    df["Threshold"])
    ...
    (df["Time"]>"2017-07-03 09:15:00")]="Sell"
    ...
    for date in sell_days:
        ...
        same_candle_exit=new_day["Time"].ix[(new_day["Close"]>new_day["SL"]) & \
        (new_day["Time"]==time_of_entry)]
        if time_of_exit.empty==False:
            if same_candle_exit.empty==False:
