...
for file in os.listdir(os.curdir):
    try:
        ...
        df=df.sort_index()
        df["mavg"]=df["Close"].rolling(window=100).mean()
        df["volavg"]=df["Volume"].rolling(window=100).mean()
        mavg_days=df["Date"].ix[(df["Close"]<df["mavg"]) & (df["Volume"]>df["volavg"]) & \
        (df["Gap_down"]=="Yes")]
